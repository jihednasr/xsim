...
{
	topology = (int**)malloc(sizeof(int*) * nb_nodes);
	for (int i=0 ; i<nb_nodes ; i++) {
		topology[i] = (int*)malloc(sizeof(int) * nb_nodes);
    ...
    if (topology_file != NULL) {
        xsim_topology_load_info(topology_file, nb_nodes);
#ifdef TOPOLOGY_INFORMATION
        for (int i=0 ; i<nb_nodes ; i++) {
            for (int j=0 ; j<nb_nodes ; j++) {
                fprintf(stdout, "node %d -> node %d : %d t \n",
                        i, j, topology[i][j]);
            }
        }
#endif
