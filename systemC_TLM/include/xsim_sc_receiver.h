...
template <int DATA_SIZE>
void 
xsim_sc_receiver<DATA_SIZE>::receive_msg_thread()
{
    ...
    while (true) {
        ...
        if (((data >> (DATA_SIZE-4)) & DSPIN_BOP) == DSPIN_BOP) {
            ...
            EMSG("[%x] Error: expected a begin of packet but does not receive one.\n",
                    ID);
            continue;
        }
